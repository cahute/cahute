...
platforms, including :ref:`system-linux` and :ref:`system-macos`.
...
``/dev/ttyUSB[0-9]+``
    USB serial devices on :ref:`system-linux`; see `USB serial on Linux`_
    for more information.

``/dev/cua[dn][0-9]+``
    Call-out ports on :ref:`system-freebsd`; see
    `Serial communications on FreeBSD`_ for more information.

``/dev/cu\..+``
    Call-out ports on :ref:`system-macos`.

``/dev/dty[0-9]+``
    Call-out ports on :ref:`system-netbsd`; see
