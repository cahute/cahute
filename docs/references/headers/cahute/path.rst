...
.. c:macro:: CAHUTE_PATH_TYPE_CASIOWIN
    ...
    * Path components must observe the 8:3 rule, i.e. respect the following
      regular expression: ``[^.]{1,8}(\.[^.]{1,3})``;
    * Uses :ref:`format-topic-text-encoding-fc8` for both the device and path components;
...
.. c:macro:: CAHUTE_PATH_TYPE_CASIOWIN_16
    ...
    character encoding is fixed to :ref:`format-topic-text-encoding-fc16`
    using the system's endianness instead of
    :ref:`format-topic-text-encoding-fc8`.
