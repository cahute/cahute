...
.. list-table::
    ...
    * - 2 B
      ...
      - 2-chars :ref:`protocol-topic-seven-ascii-hex` value, ``00`` to ``FF``
    * - 1 B
      - Extended (*EX*)
      - Whether data is attached to the packet
      - 1-char :ref:`protocol-topic-seven-ascii-hex` value, ``0`` or ``1``
...
.. list-table::
    ...
    * - *DS* B
      - Data (*D*)
      - Additional data
      - :ref:`protocol-topic-seven-5c-padding` encoded data
...
.. list-table::
    ...
    * - 2 B
      ...
      - 2-char :ref:`protocol-topic-seven-ascii-hex` value, ``00`` to ``FF``

.. _protocol-topic-seven-checksum:
...
.. _protocol-topic-seven-packet-01:
...
.. list-table::
    ...
    * - 2 (0x02)
      ...
      - Data type (*DT*)
      - Data type.
      - 2-char :ref:`protocol-topic-seven-ascii-hex` value, ``00``.
    * - 4 (0x04)
      - 8 B
      - File size (*FS*)
      - Size of the file being transferred.
      - 8-char :ref:`protocol-topic-seven-ascii-hex` value.
    * - 12 (0x0C)
      - 2 B
      - Data size 1 (*SD1*)
      - Size of *D1*
...
.. _protocol-topic-seven-packet-02:
...
been initiated following command :ref:`protocol-topic-seven-command-25`,
all data packets in the corresponding flow must bear the *ST* ``25``.
...
* If *ST* is ``00``, this is an initial check packet used to establish
  communication with the calculator. See :ref:`protocol-topic-seven-init-link`
  for more information.
* If *ST* is ``01``, this is a regular check packet used to check if
  the link is still up with the calculator. See
  :ref:`protocol-topic-seven-check-link` for more information.

This packet is not expected to be extended, i.e. *EX* should be ``0``.

.. _protocol-topic-seven-packet-06:
...
* If *ST* is ``01``, this is an overwrite confirmation; see
  :ref:`protocol-topic-seven-confirm-overwrite` for more details on the
  related flow.
* If *ST* is ``02``, this is an extended acknowledgement (EACK), providing
  ...
  **This is only used in the flow to get device information**; see
  :ref:`protocol-topic-seven-get-device-information` and
  :ref:`protocol-topic-seven-command-01` for more information.
...
.. list-table::
    ...
    * - 0 (0x00)
      ...
      - See :ref:`protocol-topic-seven-hardware-identifiers` for known
...
.. _protocol-topic-seven-packet-15:
...
* If *ST* is ``01``, the previously received packet is invalid and requires
  resending from the other side. See
  :ref:`protocol-topic-seven-report-invalid-checksum` for more information
  on the usage of this error.
* If *ST* is ``02``, the operation requires an overwrite confirmation;
  see :ref:`protocol-topic-seven-confirm-overwrite` for more details on the
  related flow.
* If *ST* is ``03``, the active side is signalling an overwrite rejection from
  the user to the passive side; see
  :ref:`protocol-topic-seven-confirm-overwrite` for more details on the related
  flow.
...
.. _protocol-topic-seven-packet-18:
...
See :ref:`protocol-topic-seven-terminate-link` for more information on the link
termination flow.
